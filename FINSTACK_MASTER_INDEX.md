...
| P12 | Timesheet + Payroll ERP | ✅ DONE | 40/40 (S1-S12) | `sql/05_phase12_erp.sql`: 6 tables — `contractor_pii` (Aadhaar/PAN/PF/bank_account stored as `bytea` via `pgp_sym_encrypt`/`erp_encrypt()` SECURITY DEFINER function; key set via `app.encrypt_key` session variable — never in schema), `timesheets` (draft→submitted→approved→billed, generated `week_end`/`total_hours` cols), `invoices` (GST auto-computed, `invoice_line_items` join), `payroll_runs` (TDS 10%+PF 12% auto-calc, generated `total_net`), `payslips`. Auto-invoice function `generate_invoice_from_timesheets()` bills approved timesheets in a period. `backend/routers/erp.py`: contractor-pii upsert/get (metadata-only, PII masked); timesheets (list/create/submit/approve); invoices (list/generate/mark-paid); payroll-runs (list/create with auto-payslip generation). Finance page timesheets/invoices/payroll tabs upgraded from stubs to live ERP tables. HARD RULE #11 verified: Aadhaar stored as 80-byte ciphertext, never returned in plaintext. zerotoken-check CLEAN (98 files). Playwright S1-S12 40/40. |
| P13 | BGV + Trust Intelligence | ✅ DONE | 46/46 (S1-S13) | `sql/06_phase13_bgv.sql`: `trust_graph` (source/target typed edges: referral/worked_with/placed/vouched/reported_fraud, weight -1.0 to 1.0, jsonb metadata), `bgv_checks` (8 types, 5-status: pending/in_progress/completed/failed/expired, result: clear/flagged/unverifiable), `bgv_documents` (file metadata, storage_path), `offer_letters` (draft_text from AI Router, e-sign refs), `v_trust_scores` view (rule-based: bgv_score from completed+clear checks, trust_graph_score from inbound edges×10, fraud_flags×-30, capped 0-100, Excellent/Good/Fair/Low/Flagged rating). `backend/routers/bgv.py`: checks CRUD, trust score, trust graph list+add-edge, offer letter draft (Tier-2 Qwen via ai_router.generate(), cache-first per HARD RULE #4), `POST /bgv/aadhaar/initiate` + `/verify-otp` (demo→auto-clears; production: UIDAI ASA), `POST /bgv/digilocker/initiate` (demo→mock OAuth URL; production: NIC). Frontend BGV page: Trust Overview (SVG score gauge, component breakdown), BGV Checks (inline initiate form, status/result badges), Trust Graph (edge table), India Verify (Aadhaar 2-step OTP + DigiLocker doc-type select). Sidebar: BGV nav item added. zerotoken-check CLEAN (100 files). Playwright S1-S13 46/46. |
| P14 | VPS Deploy (SSL + domain + production) | ✅ DONE | 52/52 (S1-S14) | `nginx/nginx.conf.template` (HTTPS redirect, SSL termination, security headers [HSTS/X-Frame-Options/X-Content-Type-Options], backend+frontend+n8n proxy with WebSocket upgrade), `docker-compose.prod.yml` override (nginx+certbot services, loopback-only port binds for backend/frontend/n8n/waha, db port removed, `ERP_ENCRYPT_KEY` required field), `.env.prod.example` (all required vars with CHANGEME placeholders, domain guidance). Scripts: `ssl-init.sh` (standalone certbot, warns against `finstack.aviinjobs.com`), `deploy-prod.sh` (preflight CHANGEME guards, zero-token audit, image pull+build, `up -d`, health-check wait), `p14-readiness-check.sh` (full checklist: env/Docker/API/DB/zero-token/SSL/Playwright). Certbot auto-renewal: 12h loop via entrypoint. **DOMAIN IS STILL TBD** — all configs use `${DOMAIN}` placeholder, must confirm with user before go-live (do NOT use `finstack.aviinjobs.com`). zerotoken-check CLEAN (103 files). Playwright S1-S14 52/52. |
