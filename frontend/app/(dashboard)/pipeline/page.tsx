...
function CandidateDrawer({ app, onClose, onMoveStage, onSubmittedToKae, onRequestReject, onRequestRemove, drawerTab, setDrawerTab, showToast, stages, allStages, requisitionId, clientName, isRecruiter, recruiterCanMoveToStage }: any) {
  ...
  // reports". Wired to the real, already-built Reminders & Follow-Ups
  // system (recruiter_tasks table, /reminders page) — not a stripped-down
  // duplicate. This lightweight count-only fetch drives the tab badge;
  // FollowUpTab (opened on click) does its own real listing fetch —
  // a small, accepted duplication matching this drawer's own existing
  // pattern for other tab badges vs. their full-list bodies.
  const { data: followupTasks } = useFetch<any[]>(app.candidate_id ? `/recruiter-tasks?candidate_id=${app.candidate_id}` : null);
  const openFollowupCount = (followupTasks || []).filter((t: any) => t.status === 'pending' || t.status === 'in_progress').length;
        ...
        <div style={{ padding: '16px 18px 0', borderBottom: '1px solid #F1F5F9', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 0, marginBottom: -1, overflowX: 'auto', flexWrap: 'nowrap' }}>
            {[
              ...
              { key: 'followup', icon: <Bell size={12} />, label: 'Follow-up', count: openFollowupCount },
