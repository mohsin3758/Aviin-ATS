...
import { UploadCloud, CheckCircle, Briefcase } from 'lucide-react';
...
export default function RecruiterJobLinkPage({ params }: { params: { token: string } }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`${API_URL}/public/job-links/${token}`);
        if (!r.ok) { const d = await r.json().catch(() => ({})); setError(d.detail || 'This link is invalid or has expired.'); setLoading(false); return; }
      ...
      <div style={wrap}>
        <div style={{ ...card, textAlign: 'center' }}>
          ...
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>
            Your resume has been sent to {info?.recruiter_name} for the {info?.requisition_title} role. They'll reach out if there's a fit.
          </p>
        </div>
      </div>
    );
